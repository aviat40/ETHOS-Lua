-- ##########################################################################################################
-- #                                                                                                        #
-- # JetsMunt lua for ETHOS V1.5.7 or above                                                                 #
-- #                                                                                                        #
-- # Turbine widget Jetsmunt telemetry module for FrSky	              			                            #
-- #                                                                                                        #
...
local localeTexts = {
	...
	ecu = {en="ECU Model", fr="Modèle ECU", de ="texte_de", es="texte_es"},
...
local function menu(widget)
	return{{"JETSMUNT V"..version, function() end}}
end
...
local function drawStatus(x, y, status)
	local statuString = msg_table_jetsmunt[status] 
...
local function configure(widget)
	-- Jetsmunt ECU Type 
    -- line = form.addLine(localize("ecu"))
	-- local ecuField = form.addChoiceField(line, nil,
		-- {{"ECU V6", 1},{"ECU V10", 2}}, 
		-- function() return widget.ecuType end,
		-- function(value) widget.ecuType = value
		-- end)
		-- Colors
...
local function read(widget)
	...
	widget.ecuType = storage.read("ecu") or 1
end
...
local function write(widget)
	...
	storage.write("ecu", widget.ecuType) 
end

local function init()
	turbineLogo = lcd.loadBitmap(bitmapsPath.."jetsmunt.png")
	...
	system.registerWidget({key="jmunt", name=name, create=create,  title=false, build=build, paint=paint, configure=configure, wakeup=wakeup, menu=menu, read=read, write=write})
end

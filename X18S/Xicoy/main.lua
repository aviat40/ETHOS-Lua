-- ##########################################################################################################
-- #                                                                                                        #
-- # Xicoy lua for X18S ETHOS V1.1.0 or above                                                               #
-- #                                                                                                        #
...
local function paint(widget)
	...
	if (w == 480) and (h == 320) then
		-- Mask
		lcd.color(common.colorMask)
		lcd.drawBitmap(150, 2, turbineLogo, 100, 50)
		lcd.drawMask(5, 5, maskTurbine)
		lcd.drawMask(10, 260, maskBattery)
		lcd.drawMask(20, 160, maskTemp)
		lcd.drawMask(10, 120, maskRpm)
		lcd.drawMask(10, 210, maskPump)
		lcd.drawMask(170, 205, maskThro)
		lcd.drawMask(170, 260, maskFuel)
		-- Datas Turbine
		lcd.color(common.colorText)
		lcd.font(FONT_XL)
		...
		if widget.sensors["value_THRT"] ~= nil then
			lcd.drawText(210, 205,widget.sensors["value_THRT"].."%",LEFT)
		else
			lcd.drawText(210, 205, "...")
		end	
			--Affichage V batterie ECU
		if widget.sensors["value_VBAT"] ~= nil then
			lcd.drawNumber(60, 265, (tonumber(widget.sensors["value_VBAT"])/10),UNIT_VOLT,1,LEFT)
		else
			lcd.drawText(60, 265, "...")
		end
			--Affichage Pump
		...
		if (widget.sensors["value_RPM2"] ~= nil) then
			...
			-- Turbine Status
		lcd.font(FONT_XXL)
		drawStatus( 180, 60, widget.sensors["value_STAT"])	
			-- Fuel Quantity
		lcd.font(FONT_XXL)
		if (widget.sensors["value_FUEL"] ~= "0") then
			...
			lcd.drawNumber(220,260, fuelRemaining,UNIT_MILLILITER)
		else
			lcd.drawText(220,260,"....")
		end
		-- Fuel Gauge Vertical
		drawGaugeV(widget)
	elseif (w == 180) and (h == 60) then
		lcd.font(FONT_XL)
		-- Fuel Quantity
		if (widget.sensors["value_FUEL"] ~= "0") then
			if (fuelRemaining < common.fuelAlarm) then
